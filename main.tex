...
Table~\ref{tab:results} reports all eight models plus the two trivial baselines, sorted by balanced accuracy. Three findings hold. First, the three highest-quality models are all open-weight---DeepSeek-V3.2 (0.674, 95\% CI $[0.653, 0.696]$), Gemma-3-4B (0.649 $[0.626, 0.673]$), and GLM-5 (0.647 $[0.629, 0.664]$)---above all three frontier models (0.613--0.623). Because balanced accuracy, not raw accuracy, is our primary metric, we test the pairwise gaps with a stratified paired bootstrap of the balanced-accuracy difference (20{,}000 resamples, resampling the 549 positive and 1000 negative task outcomes independently so the design is preserved) and control the family-wise error rate across the nine open-versus-frontier comparisons with a Holm correction. DeepSeek-V3.2 significantly exceeds all three frontier models ($\Delta$ balanced accuracy $+0.053$ to $+0.062$; Holm-adjusted $p<0.001$), and GLM-5 also exceeds all three ($+0.024$ to $+0.034$; $p<0.05$). Gemma-3-4B exceeds the two weaker frontier models GPT-5.1 and Claude-Sonnet-5 ($+0.036$, $+0.037$; $p<0.05$) but its $+0.025$ edge over the strongest frontier model, Gemini-3.1-Pro, is not significant ($p=0.06$). The claim is therefore precise: two of the three top open models beat every frontier model, and the third beats all but the strongest---not that every open model does. Indeed, one open model, Qwen3-Coder-30B, is indistinguishable from chance (balanced accuracy 0.516; MCC 0.030, 95\% CI $[-0.02, 0.08]$).
...
GLM-5 & open & 0.647 & 0.325 & 0.602 & 0.560 & 0.941 & 0.352 & 0.00062 & 471\\
...
\item \textbf{Training-data contamination.} PrimeVul is built from public CVE-fixing commits that may appear in the pretraining data of the evaluated models; if memorization differs across models it could bias the cross-tier comparison. Restricting to functions disclosed after each model's training cutoff, or to PrimeVul's paired vulnerable/patched variants (which resist superficial memorization), would bound this and is planned.
