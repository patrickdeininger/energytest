...
\item \textbf{Direct-answer configuration.} The main comparison disables reasoning where the provider allows it, for a comparable single-shot judgment. Section~\ref{sec:reasoning} quantifies the effect on a subset: reasoning buys $+0.05$--$0.08$ balanced accuracy (significant for Claude, $p=0.005$) at $4$--$7.5\times$ the cost, lifting the frontier models above the best open model, and our main-run 256-token cap understated Gemini-3.1-Pro's reasoning (it reaches $0.685$ at a 4096-token budget). A full-scale reasoning pass at an adequate budget would sharpen these frontier quality numbers, but---because reasoning multiplies cost and energy---cannot change the efficiency ordering.
