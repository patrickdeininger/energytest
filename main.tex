...
\begin{itemize}
\item \textbf{Energy is only partly measured.} We measure energy directly on an H200 GPU for the two locally-servable open models; for the three frontier models (unknown hardware) and the three open models we could not serve locally, energy remains a FLOP-based estimate. The measured points bracket that estimate within a factor of two and preserve the Pareto frontier and open-versus-frontier separation (Section~\ref{sec:energy-validation}), but a direct measurement for all self-hostable models would remove the remaining assumption.
